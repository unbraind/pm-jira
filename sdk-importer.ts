...
const defaultImportPmSdk = (): Promise<typeof import("@unbrained/pm-cli/sdk")> =>
  import("@unbrained/pm-cli/sdk");

/** The loader in effect. Production always uses {@link defaultImportPmSdk}. */
let importer: () => Promise<typeof import("@unbrained/pm-cli/sdk")> = defaultImportPmSdk;
 ...
 */
export function importPmSdk(): Promise<PmSdkModule> {
  return importer();
}
 ...
 */
export function __setPmSdkImporterForTests(replacement?: () => Promise<unknown>): void {
  importer = replacement ? () => replacement() as Promise<PmSdkModule> : defaultImportPmSdk;
}
