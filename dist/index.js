 ...
 */
export function jqlQuoteAlways(value) {
 ...
 */
export function deriveAtomicTransactionId(jql, issueKeys) {
 ...
 */
export function buildAtomicCreateMutation(item, jiraKey, transactionId, idPrefix, normalizeItemId) {
 ...
 * cannot be resolved.
 */
export async function importJiraAtomic(pmRoot, jql, filtered, opts) {
    ...
    const commit = opts.commitItemMutations
    ...
    try {
        ...
        if (settings?.id_prefix)
            idPrefix = settings.id_prefix.toString();
    }
    catch {
    ...
    const mutations = filtered.map(({ issue, item }) => buildAtomicCreateMutation(item, issue.key, transactionId, idPrefix, normalizeItemId));
    let result;
    try {
        result = await commit({
            pmRoot,
...
export async function runImport(options, pmRoot, opts = {}) {
    ...
    if (opts.issues) {
        ...
        const seamBaseUrl = (readStringOption(options, "host") ??
            (process.env["JIRA_BASE_URL"]?.trim() || "https://example.atlassian.net")).replace(/\/$/, "");
    ...
    if (atomic) {
        ...
        if (atomicResult.recovered) {
            console.error(`Atomic import resumed transaction ${atomicResult.transactionId} from a prior interrupted run (recovered ${created} item${created === 1 ? "" : "s"}).`);
        }
    }
    else {
...
function readPmItems(pmRoot) {
    ...
    const result = spawnSync("pm", ["--path", pmRoot, "--json", "list", "--full", "--include-body", "--limit", "10000"], { encoding: "utf-8", maxBuffer });
    if (result.error) {
